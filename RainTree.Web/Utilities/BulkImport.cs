...
namespace SellerVendor.Utilities
{
    public class BulkImport : BulkImportBase
    {
        ...
        public IList<ImportFileDto> ImportFile(int sellerId, byte marketPlaceId, FileType fileType, string fileName, string fileData)
        {

            var fileList = SaveDataOnServer(sellerId, fileType, fileName, fileData);
        ...
        public ImportFileDto ProcessFile(ImportFileDto file, int? sellerId)
        {
            ...
            else if (fileToProcess.FileType == FileType.SalesOrder && fileToProcess.MarketPlace == MarketPlace.Amazon)
            {
                ProcessAmazoneSalesOrder(fileToProcess.FileHistoryId);
                file.Messages.Add("Processed Successfully.");
            }
            else if (fileToProcess.FileType == FileType.Settlement && fileToProcess.MarketPlace == MarketPlace.Amazon)
            {
                ProcessAmazoneSettlement(fileToProcess.FileHistoryId);
                file.Messages.Add("Processed Successfully.");
            }
            return file;
        }
