...
namespace SellerVendor.Utilities
{
    public abstract class BulkImportBase
    {
        ...
        protected IList<ImportFileDto> SaveDataOnServer(int sellerId, FileType fileType, string fileName, string data)
        {
            ...
            importedFiles.Add(new ImportFileDto { FileName = fileName, Location = filePath, IsUploaded = true, Messages = new List<string> { "Uploaded on server." }, FileType = fileType });
            return importedFiles;
        }
        ...
        protected void DumpFileIntoDatabase(int sellerId, MarketPlace marketplaceId, FileType fileType, IEnumerable<ImportFileDto> files)
        {
            using (var context = new SellerContext())
            {
                ...
                foreach (var file in files)
                {
                    ...
                    if (importFileHistoryDetails != null && (importFileHistoryDetails.FileStatus != (byte)FileStatus.Queued && importFileHistoryDetails.FileStatus != (byte)FileStatus.Failed))
                    {
                    ...
                    var unMatchingColList = (from fileCol in columnsFromFile
                                             join dbCol in columnsFromDb
                                             on fileCol.ToUpper() equals dbCol.FileColumnName.ToUpper() into gj
                                             from unCol in gj.DefaultIfEmpty()
                                             where unCol == null
                                             select fileCol).ToList();

                    //TODO : correct it to log into database
                    if (unMatchingColList.Count > 0)
                    {
                        file.Messages.Add(unMatchingColList.Count > 0 ?
                                                   $"{string.Join(",", unMatchingColList)} columns are not matching." :
                        ...
                        context.SaveChanges();
                        throw new Exception(importFileHistoryDetails.Message);
                    }
        ...
        protected void ProcessAmazoneSettlement(int importFileHistoryId)
        {
            ProcessCommand("spProcessAmazonSettlementFile_V1", importFileHistoryId);
        }

        protected void ProcessAmazoneSalesOrder(int importFileHistoryId)
        {
            ProcessCommand("spProcessAmazonOrderFile_V1", importFileHistoryId);
        }
        ...
        private string GetDestinationTable(MarketPlace marketPlace, FileType fileType)
        {
            ...
            if (marketPlace == MarketPlace.Amazon && fileType == FileType.Settlement)
                return "AmazonSettlementImport";
            else
                return string.Empty;
        }
        private bool DumbInDatabase(string filePath, int fileHistoryId, string destinationTableName, char delimiter, IEnumerable<string> columns)
        {
            using (var conn = new MySqlConnection(ConfigurationManager.ConnectionStrings["SellerConnection"].ToString()))
            {
                try
        ...
        private List<string> GetFileColumns(string filePathAndName, char delimiter)
        {
            ...
            using (var fileStream = new FileStream(filePathAndName, FileMode.Open, FileAccess.Read, FileShare.Read))
            {
                using (StreamReader sr = new StreamReader(fileStream))
                {
                    ...
                    columnHeaders = headerRow.Split(new char[] { delimiter }, StringSplitOptions.None).Select(x => x.Replace("\r", "").Replace("\n", "").Trim()).ToList();
                }
            }
