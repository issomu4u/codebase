...
using RainTree.ServiceBroker.Services.Amazon;
...
namespace SellerVendor.Areas.Seller.Controllers
{
    public class UploadDataAPIController : Controller
    {
        ...
        public JsonResult GetApiDetails(int marketplaceid)
        {
            int valueget = 0;
            int SellerId = Convert.ToInt32(Session["SellerID"]);
            var get_sellerapi_details = dba.SellerSettingMarketplaceEntity.Where(a => a.SellerID == SellerId && a.MarketplaceID == marketplaceid && a.IsActive == 1).FirstOrDefault();
            if (get_sellerapi_details != null)
            {
                string uniqueid = get_sellerapi_details.AmazonSellerID;
                string access_key = get_sellerapi_details.AccessKeyId;
                string secret_key = get_sellerapi_details.SecretKey;
                string market_palce_id = get_sellerapi_details.AmazonMarketPlaceID;
        ...
        /// <returns></returns>
        ///       
        public JsonResult GetUploadOrderDetails1()
        {
            int sellers_id = Convert.ToInt32(Session["SellerID"]);
        ...
        public JsonResult GetUploadOrderDetails()
        {
            ...
            var get_orderupload_details = dba.ImportFileHistoryEntity.Where(a =>a.SellerId== sellers_id && a.FileType==1).OrderByDescending(a => a.Id).ToList();

            List<vieworderUpload> lst_sellermarketplace = new List<vieworderUpload>();
            foreach (var item in get_orderupload_details)
            {
                vieworderUpload uploadorder = new vieworderUpload();
                var abc = db.m_marketplace.Where(a => a.isactive == 1 && a.id == item.MarketPlaceId).FirstOrDefault();
                if (abc != null)
                {
                    uploadorder.MarketplaceName = abc.name;
                    uploadorder.ImagePath = abc.logo_path;
               ...
               // var getorder = db.
                uploadorder.FileName = item.FileName;            
                uploadorder.id = item.Id;
                uploadorder.OrderCount = "0";
                int status = item.FileStatus;
                if(status == 1)
                {
                    uploadorder.Filestatus = "Queued";
                }
                else if(status == 2)
                {
                    uploadorder.Filestatus = "Imported";
                }
                else if (status == 3)
                {
                    uploadorder.Filestatus = "Inprogress";
                }
                else if (status == 4)
                {
                    uploadorder.Filestatus = "Completed";
                }
                else if (status == 5)
                {
                    uploadorder.Filestatus = "Failed";
                }

                uploadorder.Sellerid = Convert.ToInt16(item.SellerId);
                if (item.LastUpdatedOnUtc != null)
                {
                    uploadorder.UploadedDate = Convert.ToDateTime(item.LastUpdatedOnUtc).ToString("yyyy-MM-dd");
                }                             
                uploadorder.SourceName = item.Message;
        ...
        /// </summary>
        /// <returns></returns>
        public JsonResult GetUploadTaxDetails1()
        {
        ...
        public JsonResult GetUploadTaxDetails()
        {
            ...
            var get_taxupload_details = dba.ImportFileHistoryEntity.Where(a => a.SellerId == sellers_id && a.FileType == 3).OrderByDescending(a => a.Id).ToList();
        ...
        public JsonResult GetSettlementOrderDetails1()
        {
            int sellers_id = Convert.ToInt32(Session["SellerID"]);
            var get_settlementupload_details = dba.tbl_settlement_upload.Where(a => a.tbl_seller_id == sellers_id).OrderByDescending(a => a.Id).ToList();
        ...
        public ActionResult SettlementAPI(int? ddl_marketplaceSettAPI, DateTime? txt_Settlementfrom, DateTime? txt_settlementto)
        {
            ...
            //TODO : Settlement form Amazon place under market place.
            ProcessAmazonSettlement(sellerId, ddl_marketplaceSettAPI);
        ...
        public JsonResult ProcessAmazonSettlement(int sellerId ,int? ddl_marketplaceSettAPI)
        {
            var getdata = dba.SellerSettingMarketplaceEntity.Where(a => a.SellerID == sellerId && a.MarketplaceID == ddl_marketplaceSettAPI).FirstOrDefault();
            if(getdata!= null)
            {
                var sellerParam = new AmzaonParamDto
                {
                    MarketPlaceId = getdata.AmazonMarketPlaceID,//"A21TJRUUN4KGV",
                    AWSAccessKeyId = getdata.AccessKeyId,//"AKIAIJJRZU64IB3R5YQQ",
                    Domain = "https://mws.amazonservices.in",
                    SecretKey = getdata.SecretKey,//"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",
                    SellerId = getdata.AmazonSellerID,//"A89M6K3D9CYQY",
                    SignatureMethod = "HmacSHA256",
                    SignatureVersion = "SignatureVersion"
                };
                var settlementService = new AmazoneSettlementReportService();

                var data = settlementService.GetSettlementDataFromCsvReport(sellerParam, "11542664662017725");
                 if (string.IsNullOrWhiteSpace(data))
                return Json("Process Completed", JsonRequestBehavior.AllowGet);

                var bulkImport = new BulkImport();
                var filesResult= bulkImport.ImportFile(sellerId, (byte)MarketPlace.Amazon, FileType.Settlement, "11542664662017725.csv", data);
