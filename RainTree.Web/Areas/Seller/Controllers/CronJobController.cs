    ...
    public class CronJobController : Controller
    {
            ...
            using (settlementDB_Context = new SellerContext())
            {
                ...
                try
                {
                    ...
                    if (objjson1 != null)
                    {
                        ...
                        var diff_order = 0;
                        int totalOrder = 0;
                        var get_balance_details = dba.tbl_user_login.Where(a => a.tbl_sellers_id == SellerId).FirstOrDefault();
                        if (get_balance_details != null)
                        {
                            ...
                            dba.Entry(get_balance_details).State = EntityState.Modified;
                            dba.SaveChanges();
        ...
        public int bulkxml_Parse(string strFilePath, int marketplaceID, int id, int SellerId)
        {
            ...
            try
            {
                                                ...
                                                foreach (var details in obj1)
                                                {
                                                    ...
                                                    if (get_saleorderdetails != null)
                                                    {
                                                        ...
                                                        if (get_sales_orders != null)
                                                        {
                                                            ...
                                                            var seller_details = dba.tbl_sellers.Where(t => t.id == SellerId).FirstOrDefault();
                                                            ...
                                                            {
                                                                var getcountrydetails = dba.tbl_country.Where(t => t.status == 1 && t.countrylevel == 0 && t.id == seller_details.country).FirstOrDefault();// to get country name from country table in admin db.
                                                                var getstatedetails = dba.tbl_country.Where(m => m.id == seller_details.state && m.countrylevel == 1 && m.status == 1).FirstOrDefault();//to get state name from country table in admin db.
                ...
                var get_balance_details = dba.tbl_user_login.Where(aa => aa.tbl_sellers_id == SellerId).FirstOrDefault();
                if (get_balance_details != null)
                {
                    var last_order = get_balance_details.total_orders;
                    diff_order = Convert.ToInt16(uniqueupload - last_order);
        ...
        public string GetFlipkartSalesOrderSearchData(int? ddl_marketplaceAPI, DateTime? txt_from, DateTime? txt_to, int sellers_id, int? id)
        {
            ...
            try
            {
                ...
                string token = "";
                var seller_List_Admin = dba.tbl_sellers.Where(a => a.isactive == 1 && a.id == sellers_id).FirstOrDefault();
            ...
            try
            {
                ...
                if (objjson != null)
                {
                    ...
                    foreach (var item in objjson[0].orderItems)
                    {
                        string Message;
                        bool flag = false;
                        var getmarketplacedetails = dba.m_marketplace.Where(a => a.isactive == 1).ToList();// get details from marketplace table in seller admin
                    ...
                    var get_balance_details = dba.tbl_user_login.Where(aa => aa.tbl_sellers_id == sellers_id).FirstOrDefault();
                    if (get_balance_details != null)
                    {
                        var last_order = get_balance_details.total_orders;
                        diff_order = Convert.ToInt16(uniqueupload - last_order);
