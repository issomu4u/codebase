...
namespace SellerVendor.Areas.Seller.Controllers
{
    public class SellerController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            var get_allsaleorders = dba.MarketPlaceOrderEntity.Where(a => a.SellerId == SellerId).Count();
        ...
        public JsonResult GetSellermarketDetails()
        {
            ...
            bool ss = cf.session_check();
            int sellers_id = Convert.ToInt32(Session["SellerID"]);
            var get_details = (from ob_SellerSettingMarketplaceEntity in dba.SellerSettingMarketplaceEntity
                               select new SellerUtility
                               {
                                   ob_SellerSettingMarketplaceEntity = ob_SellerSettingMarketplaceEntity

                               }).Where(a => a.ob_SellerSettingMarketplaceEntity.IsActive == 1 && a.ob_SellerSettingMarketplaceEntity.SellerID == sellers_id).OrderByDescending(a => a.ob_SellerSettingMarketplaceEntity.ID).ToList();
            ...
            foreach (var item in get_details)
            {
                ...
                var abc = db.m_marketplace.Where(a => a.isactive == 1 && a.id == item.ob_SellerSettingMarketplaceEntity.MarketplaceID).FirstOrDefault();
                if (abc != null)
                {
                    sellermarketplace.MarketplaceID = abc.id;
                    sellermarketplace.MarketplaceName = abc.name;
                    sellermarketplace.ImagePath = abc.logo_path;
                }
                sellermarketplace.AmazonSellerID = item.ob_SellerSettingMarketplaceEntity.AmazonSellerID;
                
                sellermarketplace.SecretKey = item.ob_SellerSettingMarketplaceEntity.SecretKey;               
                sellermarketplace.AccessKeyId = item.ob_SellerSettingMarketplaceEntity.AccessKeyId;
                sellermarketplace.id = item.ob_SellerSettingMarketplaceEntity.ID;
                sellermarketplace.AmazonMarketPlaceID = item.ob_SellerSettingMarketplaceEntity.AmazonMarketPlaceID;
                sellermarketplace.GSTNNo = item.ob_SellerSettingMarketplaceEntity.GSTNNo;
        ...
        public JsonResult DeleteSellerMarket(string id)
        {
            ...
            {
                int no = Convert.ToInt32(id);
                SellerSettingMarketplaceEntity objtbl = dba.SellerSettingMarketplaceEntity.Where(a => a.ID == no).FirstOrDefault();
                objtbl.IsActive = 0;
        ...
        [HttpPost]
        public JsonResult SaveMarketPlaceDetails(SellerSettingMarketplaceEntity obj)
        {
            cf = new comman_function();
            bool ss = cf.session_check();
            int sellers_id = Convert.ToInt32(Session["SellerID"]);
            ...
            {
                //string Message;
                var get_details = dba.tbl_sellermarketplace.Where(a => a.m_marketplace_id == obj.MarketplaceID && a.tbl_seller_id == sellers_id && a.isactive == 1).FirstOrDefault();
                if (get_details == null)
                {
                    var get_marketplace = db.m_marketplace.Where(a => a.id == obj.MarketplaceID && a.isactive == 1).FirstOrDefault();
                    if (get_marketplace != null)
                    {
                        obj.MarketPlaceName = get_marketplace.name;
                    }
                    obj.IsActive = 1;
                    obj.SellerID = sellers_id;
                    obj.Createdon = DateTime.Now;
                    dba.SellerSettingMarketplaceEntity.Add(obj);
                    dba.SaveChanges();
                    flag = true;
        ...
        public JsonResult SaveMarketPlaceDetails1(tbl_sellermarketplace obj)
        {
            cf = new comman_function();
            bool ss = cf.session_check();
            int sellers_id = Convert.ToInt32(Session["SellerID"]);
        ...
        public JsonResult UpdateDetails(viewsellermarketplace objm)
        {
            ...
            try
            {
                var get_details = dba.SellerSettingMarketplaceEntity.Where(a => a.ID == objm.id).FirstOrDefault();
                if (get_details != null)
                {
                    get_details.MarketplaceID = objm.MarketplaceID;
                    get_details.AmazonSellerID = objm.AmazonSellerID;
                    //get_details.t_loginName = objm.t_loginName;
                    //get_details.t_password = objm.t_password;
                    get_details.AccessKeyId = objm.AccessKeyId;
                   // get_details.t_auth_token = objm.t_auth_token;
                    get_details.SecretKey = objm.SecretKey;
                    get_details.AmazonMarketPlaceID = objm.AmazonMarketPlaceID;
                    get_details.GSTNNo = objm.GSTNNo;
                    dba.Entry(get_details).State = EntityState.Modified;
                    dba.SaveChanges();
                    flag = true;
