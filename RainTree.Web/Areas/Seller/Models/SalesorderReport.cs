...
namespace SellerVendor.Areas.Seller.Models
{
    public class SalesorderReport
    {
        ...
        public string Get_AmazonDebtorLedger(List<SaleReport> lstOrdertext2, SaleReport view_salereport, int? ddl_marketplace, DateTime? txt_from, DateTime? txt_to, int sellers_id)
        {
            ...
            try
            {
                if (txt_from != null && txt_to != null)
                {
                    ...
                    var get_marketplace = dba.m_marketplace.Where(a => a.isactive == 1).ToList();
        ...
        public string Get_SalesReport_Paytm_old(List<SaleReport> lstOrdertext2, SaleReport view_salereport, DateTime? txt_from, DateTime? txt_to, int sellers_id, int? ddl_market_place, int? ddl_percentage)
        {
            string Msg = "S";
            try
            {
        ...
        public string Get_SalesReport_Paytm(List<SaleReport> lstOrdertext2, SaleReport view_salereport, DateTime? txt_from, DateTime? txt_to, int sellers_id, int? ddl_market_place, int? ddl_percentage)
        {
            ...
            try
            {
                if (txt_from != null && txt_to != null)
                {
                    var get_order = dba.MarketPlaceOrderEntity.Where(a => a.SellerId == sellers_id && a.MarketPlaceId == ddl_market_place ).ToList();

                    if (get_order != null)
                    {
                        get_order = get_order.Where(a => a.MarketPlaceOrderDate >= txt_from && a.MarketPlaceOrderDate <= txt_to).ToList();
                        if (get_order != null)
                        {
                            foreach (var item in get_order)
                            {
                                view_salereport = new SaleReport();
                                string order_id = item.MarketPlaceOrderId;
                                int id = item.Id;

                                view_salereport.OrderID = item.MarketPlaceOrderId;
                                view_salereport.OrderDate = Convert.ToDateTime(item.MarketPlaceOrderDate).ToString("yyyy-MM-dd");

                                #region to getorder
                                var get_orderitem = dba.MarketPlaceOrderItemEntity.Where(a => a.OrderId == id).FirstOrDefault();
                                if (get_orderitem != null)
                                {
                                    int itemid = Convert.ToInt16(get_orderitem.ItemId);
                                    view_salereport.DispatchDate = Convert.ToDateTime(get_orderitem.ShippingDate).ToString("yyyy-MM-dd");
                                    view_salereport.Principal = Convert.ToDouble(get_orderitem.ItemPriceWithoutTax);

                                    var getitem_details = dba.ItemEntity.Where(a => a.SellerId == sellers_id && a.MarketPlaceId == ddl_market_place && a.Id == itemid).FirstOrDefault();
                                    if (getitem_details != null)
                                    {
                                        view_salereport.ProductName = getitem_details.ItemName;
                                        view_salereport.skuNo = getitem_details.SellerSku;

                                    }// end of getitem_details

                                    var getorder_itemtransaction = dba.MarketPlaceOrderItemTransactionEntity.Where(a => a.OrderItemId == get_orderitem.Id).FirstOrDefault();
                                    if (getorder_itemtransaction != null)
                                    {
                                        decimal igstamt = 0, sgstamt = 0, cgstamt = 0;
                                        if (getorder_itemtransaction.IgstAmount != null)
                                        {
                                            igstamt = Convert.ToDecimal(getorder_itemtransaction.IgstAmount);
                                        }
                                        if (getorder_itemtransaction.SgstAmount != null)
                                        {
                                            sgstamt = Convert.ToDecimal(getorder_itemtransaction.SgstAmount);
                                        }
                                        if (getorder_itemtransaction.CgstAmount != null)
                                        {
                                            cgstamt = Convert.ToDecimal(getorder_itemtransaction.CgstAmount);
                                        }
                                        view_salereport.orderigst = Convert.ToDouble(igstamt);
                                        view_salereport.ordersgst = Convert.ToDouble(sgstamt);
                                        view_salereport.ordercgst = Convert.ToDouble(cgstamt);
                                    }
                                    view_salereport.ordertotal = view_salereport.Principal + view_salereport.orderigst + view_salereport.ordersgst + view_salereport.ordercgst;
                                ...
                                var get_settlemntitemss = dba.MarketPlaceSettlementItemEntity.Where(a => a.MarketPlaceOrderId == order_id).ToList();
                                if (get_settlemntitemss != null)
                                {
                                    foreach (var detail in get_settlemntitemss)
                                    {
                                ...
                                #region getsettlementdetails

                                //var get_settlemnt = dba.MarketPlaceSettlementEntity.Where(a => a.SellerId == sellers_id && a.MarketPlaceId == ddl_market_place).ToList();
                                //if(get_settlemnt != null)
                                //{
                                //    get_settlemnt = get_settlemnt.Where(a => a.SettlementDate >= txt_from && a.SettlementDate <= txt_to).ToList();

                                //    if(get_settlemnt != null)
                                //    {
                                var get_settlemntitem = dba.MarketPlaceSettlementItemEntity.Where(a => a.MarketPlaceOrderId == order_id).FirstOrDefault();
                                if (get_settlemntitem != null)
                                {
                                    var reference = dba.MarketPlaceSettlementEntity.Where(a => a.SellerId == sellers_id && a.MarketPlaceId == ddl_market_place && a.Id == get_settlemntitem.SettlementId).FirstOrDefault();
                                    view_salereport.orderprincipal = Convert.ToDouble(get_settlemntitem.OrderItemPrice);

                                    view_salereport.orderTotal = view_salereport.orderprincipal;
                                    view_salereport.ReferenceID = reference.SettlementReferenceNumber;

                                    double FBAFEE = 0, FBAIGST = 0, FBACGST = 0, FBASGST = 0, TechnologyFee = 0, TechnologyIGST = 0, TechnologyCGST = 0, TechnologySGST = 0, CommissionFee = 0, CommissionIGST = 0, CommissionCGST = 0, CommissionSGST = 0, FixedClosingFee = 0, FixedclosingIGST = 0,
                                               FixedclosingCGST = 0, FixedclosingSGST = 0, ShippingChargebackFee = 0,
                                               shippingchargeCGST = 0, shippingchargeSGST = 0, shippingchargeIGST = 0;

                                    var gettransaction = dba.MarketPlaceSettlementItemTransactionEntity.Where(a => a.SettlementItemId == get_settlemntitem.Id && a.TransactionTypeId == 2).ToList();
                                    if (gettransaction != null && gettransaction.Count > 0)
                                    {
                                        foreach (var exp in gettransaction)
                                        {
                                            var exp_id = exp.MarketPlaceTransactionTypeId;
                                            var get_expdetails = dba.MarketPlaceTransactionTypeEntity.Where(a => a.Id == exp_id).FirstOrDefault();
                                            if (get_expdetails != null)
                                            {
                                                string nam = get_expdetails.RainTreeDescription;
                                                if (nam == "Marketplace Commission")
                                                {
                                                    if (get_expdetails.Id == exp_id)
                                                    {
                                                        FBAFEE = FBAFEE + Convert.ToDouble(exp.Amount);
                                                        view_salereport.FBAFEE = FBAFEE;

                                                        FBAIGST = FBAIGST + Convert.ToDouble(exp.IgstAmount);//new
                                                        FBACGST = FBACGST + Convert.ToDouble(exp.CgstAmount);
                                                        FBASGST = FBASGST + Convert.ToDouble(exp.SgstAmount);
                                                        view_salereport.FBACGST = FBACGST;
                                                        view_salereport.FBASGST = FBASGST;
                                                        view_salereport.FBAIGST = FBAIGST;//new                                                            
                                                    }
                                                ...
                                                else if (nam == "Logistics Charges")
                                                {
                                                    if (get_expdetails.Id == exp_id)
                                                    {
                                                        TechnologyFee = TechnologyFee + Convert.ToDouble(exp.Amount);
                                                        view_salereport.TechnologyFee = TechnologyFee;

                                                        TechnologyIGST = TechnologyIGST + Convert.ToDouble(exp.IgstAmount);
                                                        TechnologyCGST = TechnologyCGST + Convert.ToDouble(exp.CgstAmount);
                                                        TechnologySGST = TechnologySGST + Convert.ToDouble(exp.SgstAmount);
                                                ...
                                                else if (nam == "PG Commission")
                                                {
                                                    if (get_expdetails.Id == exp_id)
                                                    {
                                                        CommissionFee = CommissionFee + Convert.ToDouble(exp.Amount);
                                                        view_salereport.CommissionFee = CommissionFee;

                                                        CommissionIGST = CommissionIGST + Convert.ToDouble(exp.IgstAmount);
                                                        CommissionCGST = CommissionCGST + Convert.ToDouble(exp.CgstAmount);
                                                        CommissionSGST = CommissionSGST + Convert.ToDouble(exp.SgstAmount);
                                                        view_salereport.CommissionIGST = CommissionIGST;
                                                        view_salereport.CommissionCGST = CommissionCGST;
                                                        view_salereport.CommissionSGST = CommissionSGST;
                                                ...
                                                else if (nam == "Penalty")
                                                {
                                                    if (get_expdetails.Id == exp_id)
                                                    {
                                                        FixedClosingFee = FixedClosingFee + Convert.ToDouble(exp.Amount);
                                                        view_salereport.FixedClosingFee = FixedClosingFee;

                                                        FixedclosingIGST = FixedclosingIGST + Convert.ToDouble(exp.IgstAmount);
                                                        FixedclosingCGST = FixedclosingCGST + Convert.ToDouble(exp.CgstAmount);
                                                        FixedclosingSGST = FixedclosingSGST + Convert.ToDouble(exp.SgstAmount);
                                                ...
                                                else if (nam == "Net Adjustments")
                                                {
                                                    if (get_expdetails.Id == exp_id)
                                                    {
                                                        ShippingChargebackFee = ShippingChargebackFee + Convert.ToDouble(exp.Amount);
                                                        view_salereport.ShippingChargebackFee = ShippingChargebackFee;

                                                        shippingchargeIGST = shippingchargeIGST + Convert.ToDouble(exp.IgstAmount);
                                                        shippingchargeCGST = shippingchargeCGST + Convert.ToDouble(exp.CgstAmount);
                                                        shippingchargeSGST = shippingchargeSGST + Convert.ToDouble(exp.SgstAmount);
                                                        view_salereport.shippingchargeCGST = shippingchargeCGST;
                                                        view_salereport.shippingchargeSGST = shippingchargeSGST;
                                                        view_salereport.shippingchargeIGST = shippingchargeIGST;
                                                    }
                                ...
                                #region getrefund
                                //var get_settlemnt1 = dba.MarketPlaceSettlementEntity.Where(a => a.SellerId == sellers_id && a.MarketPlaceId == ddl_market_place).ToList();
                                //if (get_settlemnt1 != null)
                                //{
                                //    get_settlemnt1 = get_settlemnt1.Where(a => a.SettlementDate >= txt_from && a.SettlementDate <= txt_to).ToList();

                                //    if (get_settlemnt1 != null)
                                //    {
                                var get_settlemntitem1 = dba.MarketPlaceSettlementItemEntity.Where(a => a.MarketPlaceOrderId == order_id).FirstOrDefault();
                                if (get_settlemntitem1 != null)
                                {
                                    if (get_settlemntitem1.ReturnAmount != 0)
                                    {
                                        var reference = dba.MarketPlaceSettlementEntity.Where(a => a.SellerId == sellers_id && a.MarketPlaceId == ddl_market_place && a.Id == get_settlemntitem1.SettlementId).FirstOrDefault();
                                        view_salereport.refundprincipal = Convert.ToDouble(get_settlemntitem1.OrderItemPrice);


                                        view_salereport.refundTotal = view_salereport.refundprincipal;

                                        view_salereport.refundReferenceID = reference.SettlementReferenceNumber;
                                        ...
                                        var gettransaction = dba.MarketPlaceSettlementItemTransactionEntity.Where(a => a.SettlementItemId == get_settlemntitem1.Id && a.TransactionTypeId == 1).ToList();
                                        if (gettransaction != null && gettransaction.Count > 0)
                                        {
                                            foreach (var exp in gettransaction)
                                            {
                                                ...
                                                if (get_expdetails != null)
                                                {
                                                    ...
                                                    if (nam == "Marketplace Commission")
                                                    {
                                                        if (get_expdetails.Id == exp_id)
                                                        {
                                                            refundFBAFEE = refundFBAFEE + Convert.ToDouble(exp.Amount);
                                                            view_salereport.RefundFBAFEE = refundFBAFEE;

                                                            refundFBAIGST = refundFBAIGST + Convert.ToDouble(exp.IgstAmount);
                                                            refundFBACGST = refundFBACGST + Convert.ToDouble(exp.CgstAmount);
                                                            refundFBASGST = refundFBASGST + Convert.ToDouble(exp.SgstAmount);
                                                    ...
                                                    else if (nam == "Logistics Charges")
                                                    {
                                                        if (get_expdetails.Id == exp_id)
                                                        {
                                                            refundTechnologyFee = refundTechnologyFee + Convert.ToDouble(exp.Amount);
                                                            view_salereport.RefundTechnologyFee = refundTechnologyFee;

                                                            refundTechnologyIGST = refundTechnologyIGST + Convert.ToDouble(exp.IgstAmount);
                                                            refundTechnologyCGST = refundTechnologyCGST + Convert.ToDouble(exp.CgstAmount);
                                                            refundTechnologySGST = refundTechnologySGST + Convert.ToDouble(exp.SgstAmount);
                                                    ...
                                                    else if (nam == "PG Commission")
                                                    {
                                                        if (get_expdetails.Id == exp_id)
                                                        {
                                                            refundCommissionFee = refundCommissionFee + Convert.ToDouble(exp.Amount);
                                                            view_salereport.RefundCommissionFee = refundCommissionFee;

                                                            refundCommissionIGST = refundCommissionIGST + Convert.ToDouble(exp.IgstAmount);
                                                            refundCommissionCGST = refundCommissionCGST + Convert.ToDouble(exp.CgstAmount);
                                                            refundCommissionSGST = refundCommissionSGST + Convert.ToDouble(exp.SgstAmount);
                                                    ...
                                                    else if (nam == "Penalty")
                                                    {
                                                        if (get_expdetails.Id == exp_id)
                                                        {
                                                            refundFixedClosingFee = refundFixedClosingFee + Convert.ToDouble(exp.Amount);
                                                            view_salereport.RefundFixedClosingFee = refundFixedClosingFee;

                                                            refundFixedclosingIGST = refundFixedclosingIGST + Convert.ToDouble(exp.IgstAmount);
                                                            refundFixedclosingCGST = refundFixedclosingCGST + Convert.ToDouble(exp.CgstAmount);
                                                            refundFixedclosingSGST = refundFixedclosingSGST + Convert.ToDouble(exp.SgstAmount);
                                                    ...
                                                    else if (nam == "Net Adjustments")
                                                    {
                                                        if (get_expdetails.Id == exp_id)
                                                        {
                                                            refundShippingChargebackFee = refundShippingChargebackFee + Convert.ToDouble(exp.Amount);
                                                            view_salereport.RefundShippingChargebackFee = refundShippingChargebackFee;

                                                            refundshippingchargeIGST = refundshippingchargeIGST + Convert.ToDouble(exp.IgstAmount);
                                                            refundshippingchargeCGST = refundshippingchargeCGST + Convert.ToDouble(exp.CgstAmount);
                                                            refundshippingchargeSGST = refundshippingchargeSGST + Convert.ToDouble(exp.SgstAmount);
                                                            view_salereport.RefundshippingchargeCGST = refundshippingchargeCGST;
                                                            view_salereport.RefundshippingchargeSGST = refundshippingchargeSGST;
                                                            view_salereport.RefundshippingchargeIGST = refundshippingchargeIGST;
